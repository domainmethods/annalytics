...
The codebase is healthy: ~9.5K source lines, 897 tests passing, typecheck clean, module boundaries respected, no dead exports. **The measurement this document gated everything on has now been taken**: the first real ReferenceCard acceptance run was recorded 2026-06-11 with verdict `NEEDS_REVISION` (Evidence Log). The dominant risk is no longer instruments-without-measurement — it is scope drift during repair: treating the repair tranche as license to expand domains, scaffolding, or features before a re-run records `ACCEPTED`.

| Area | Status |
|---|---|
| Core pipeline (intake → clarification → SQL → supervisor → L1–L4 validation → execution → respond) | Live, template-complete |
| ReferenceCard v1 + acceptance analyzer | **First real acceptance run recorded 2026-06-11** (sessions & traffic attribution pilot, 26-question corpus): verdict `NEEDS_REVISION` — 4 failing entries in 2 categories (clarification-gate coverage, SQL shape); 21/23 non-ambiguous questions passed the quality loop with correct table selection throughout. Evidence operator-local and gitignored per the template boundary; decision in the Evidence Log. |
...
| Confidence calibration | First real calibration verdict recorded 2026-06-11 (judge enrichment run on the acceptance data): `FAIL` on bucket coverage only — the 26-question slice produced no medium-confidence results, so the monotonicity rule cannot evaluate; the substance is strongly monotonic (low 80% wrong, high 0% wrong). Side bar stays deferred per T3 #1; the repair re-run is the next chance at a complete-bucket slice |
...
1. **The active product tranche is the T2 repair branch taken on 2026-06-11**: scoped repair of the two evidence categories the first acceptance run failed — (a) clarification-gate coverage of session-mart metrics no card mentions (or a recorded out-of-domain decision and corpus re-slice), and (b) SQL-shape conformance (source-vs-channel conflation; mart-column-vs-staging-table) — followed by a benchmark re-run. Domain expansion stays blocked until a re-run records `ACCEPTED` (Tranche A's branch rule, unchanged).
2. **Evaluation-scaffolding freeze: blocking condition satisfied 2026-06-11.** The freeze's condition — the first real acceptance decision recorded — is now met, so the categorical prohibition lifts. But per the Tranche Horizon (T3 #4), net-new benchmark/calibration/sizing/sweep machinery still activates **last**, only once evidence is flowing routinely — at minimum this decision plus one post-acceptance benchmark slice (the repair re-run qualifies). The operative rule is unchanged in practice: fixing defects in existing instruments that the acceptance run surfaces is fine; net-new measurement capability is not the next tranche — the repair is.
...
1. **Side bar calibration check.** Run the calibration verdict against the real benchmark/judge data the acceptance run produced. If it passes, the side bar pilot (admin suspend/consult/resume behind a config flag) becomes the next product tranche; if it fails, the side bar stays deferred and the failure is recorded here. **Outcome (2026-06-11): `FAIL` — side bar stays deferred.** The failure is bucket coverage, not miscalibration: the run yielded only low (5 results, 80% wrong) and high (21 results, 0% wrong) buckets — monotonic in the trustworthy direction with a 0.80 low-high delta — but no medium-confidence results at all, so the `low >= medium >= high` rule cannot evaluate. The check re-arms on the repair re-run (or any future slice) that produces all three buckets at `minSample = 5`.
...
## Tranche A — Implementation ReferenceCard Acceptance Run (first run recorded 2026-06-11)

**Status (2026-06-11):** executed and recorded — verdict `NEEDS_REVISION` (Evidence Log). All acceptance criteria below were met (real benchmark JSON, both analyzer reports, a verdict, and this document's record). The active tranche is now the scoped repair branch (Current Decision #1); the section is retained as the spec the re-run must satisfy.
...
| New evaluation/benchmark/sizing scaffolding | Freeze condition met 2026-06-11, but T3 #4 orders the unfreeze last | Evidence flowing routinely: the 2026-06-11 decision **plus** one post-acceptance benchmark slice (the repair re-run qualifies) |
