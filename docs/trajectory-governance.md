...
The codebase is healthy: ~8.8K source lines, 813 tests passing, typecheck clean, module boundaries respected, no dead exports. The dominant risk is no longer code quality — it is that the project keeps building measurement instruments instead of taking the measurement its own gate requires.
...
| Operational trust (Firestore retention, telemetry sink, escalation timers) | **Gaps found by the 2026-06-09 audit — now sanctioned as Active Tranche B** |
...
4. **The escalation card promises a feature that does not exist.** `src/slack/escalationBlocks.ts:60` tells analysts "React with ✅ if my guess is correct" but no `reaction_added` handler is registered anywhere. Required: either ship the reaction handler (it is the analyst quick-path the card was designed around) or remove the copy. Shipping a promise and not the feature is a trust defect by this document's own standard.

Acceptance criteria: TTL manifest exists and the apply path is documented; usage records visible in logs from a deployed instance; an escalation in a zero-traffic workspace times out and notifies the user; the ✅ promise is either honored or gone.

## Sanctioned UX-Trust Items

These are prerequisites for the 2026-06-07 feedback-sensor strategy, not feature expansion. That strategy bets on users continuing to supply feedback; users stop feeding a sensor that never responds.

1. **Close the feedback loop to the user.** Today 👎 → reason → teaching candidate → human approval → sync happens entirely silently from the reporting user's perspective. When a teaching candidate that originated from a user's feedback is promoted, notify that user in the originating thread. This is the cheapest retention mechanism the feedback sensor has.
