...
Current guidance: prioritize trust infrastructure over feature expansion. The `ReferenceCard v1` acceptance gate **passed**: after two `NEEDS_REVISION` runs (2026-06-11) and a confirming slice, the post-repair re-run recorded **`ACCEPTED`** 2026-06-12 with zero scorecard failures (sessions & traffic attribution pilot — see the governance Evidence Log). The one-additional-domain branch is taken: the next product tranche may add exactly one high-confusion ReferenceCard domain, but selection must come from aggregated production feedback signal (not analyst convenience) and the new domain needs its own benchmark slice before it ships — so it waits on feedback data, not engineering. Tranche B (operational trust maintenance — Firestore TTL/retention, production telemetry sink, time-driven escalation timeouts, the escalation ✅-reaction handler) was completed 2026-06-09. The evaluation-scaffolding freeze is lifted, but net-new benchmark/calibration/sizing/sweep machinery still activates last in the Tranche Horizon (behind the side bar calibration check, teaching impact measurement, and node sizing); fixing defects in existing instruments remains fine. The WhatsApp Cloud API channel is a gated prototype (default-off, allowlisted; interactive trust controls included since 2026-06-23) — **further WhatsApp tranches are capped until manual acceptance evidence is recorded in the governance doc** (defect fixes in the shipped surface exempt); any change touching a surface named in the governance doc's Deferred Work table must carry its governance update in the same change set. Do not commit client-specific dbt artifacts, project IDs, File Search store IDs, ReferenceCards, or benchmark evidence to this template unless the repository has intentionally become an implementation repo. Do not revive broad charts, BQML expansion, domain agents, or automatic correction harvesting as the active next tranche unless `docs/trajectory-governance.md` is updated first with the new rationale and evidence.
...
| `whatsapp_event_dedupe` | event doc id | WhatsApp webhook delivery dedupe (gated prototype); `expiresAt` TTL |
| `whatsapp_action_context` | action context id | Answer context for WhatsApp interactive controls (feedback/reasoning/SQL/format follow-ups); `expiresAt` TTL (24h) |
| `whatsapp_pending_feedback_notes` | `conversationId` | Pending WhatsApp negative-feedback free-text capture; `expiresAt` TTL (30m) |
...
| GET | `/whatsapp/webhook` | `hub.verify_token` match | Meta webhook verification handshake (gated WhatsApp prototype; registered only when `WHATSAPP_ENABLED=true`) |
| POST | `/whatsapp/webhook` | `X-Hub-Signature-256` (HMAC, `WHATSAPP_APP_SECRET`) | Inbound WhatsApp messages + interactive replies (gated prototype; allowlisted senders only) |
