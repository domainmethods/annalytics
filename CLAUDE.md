...
Current guidance: prioritize trust infrastructure over feature expansion. The first `ReferenceCard v1` acceptance run was recorded 2026-06-11 with verdict `NEEDS_REVISION` (sessions & traffic attribution pilot — see the governance Evidence Log). The active tranche is the scoped repair of its two failing evidence categories — clarification-gate coverage of uncovered session-mart metrics, and SQL-shape conformance — followed by a benchmark re-run; domain expansion stays blocked until a re-run records `ACCEPTED`. Tranche B (operational trust maintenance — Firestore TTL/retention, production telemetry sink, time-driven escalation timeouts, the escalation ✅-reaction handler) was completed 2026-06-09. The categorical evaluation-scaffolding freeze lifted with the recorded decision, but net-new benchmark/calibration/sizing/sweep machinery still activates last in the Tranche Horizon (after the repair re-run produces a second evidence slice); fixing defects in existing instruments remains fine. Do not commit client-specific dbt artifacts, project IDs, File Search store IDs, ReferenceCards, or benchmark evidence to this template unless the repository has intentionally become an implementation repo. Do not revive broad charts, BQML expansion, domain agents, or automatic correction harvesting as the active next tranche unless `docs/trajectory-governance.md` is updated first with the new rationale and evidence.
